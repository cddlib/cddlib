...
%KF2021 add url and remove html
...
\author{Komei Fukuda\\
%KF2021 update the affiliation
%Institute for Operations Research\\
Department of Mathematics\\
and Institute of Theoretical Computer Science\\
ETH Zentrum, CH-8092 Zurich, Switzerland\\
}
%KF2021 date modified
\date{ (cddlib ver. 0.94m, manual ver. February 18, 2021)}
...
\begin{abstract}
This is a reference manual for cddlib-094m.
...
programs such as scdd.c, testlp*.c and testcdd*.c
in the {\tt src} subdirectory of the source
distribution.
...
the solutions in {\tt *lp}.  Unlike the earlier versions
...
%KF2021 This function works now, and is documented.
\item[{\tt dd\_rowset dd\_RedundantRowsViaShooting(matrix, err) }]:\\
Returns a maximal set of row indices such that the associated rows
can be eliminated without changing the polyhedron.  It implements
an efficient algorithm due to Clarkson, see \cite[Chapter 7]{f-pc-20}.
At the moment, Clarkson's algorithm is applied only when the input is 
a full-dimensional H-polyhedron.  When the H-polyhedron is not full-dimensional,
 it calls a slower {\tt dd\_RedundantRows}.
Experimental comparisons reported at\\
\url{https://people.inf.ethz.ch/fukudak/ClarksonExp/ExperimentCube.html}\\
indicate a huge acceleration for highly redundant cases. \\When the input is V-representation,
this function treats it as if it was an H-representation.  Thus, it may not work correctly, but 
it might work for some cases,
for example, when the input is a homogeneous cone.
...
in the {\tt src}
%KF2021 delete this line
%and {\tt src-gmp} subdirectories 
...
\[
  ...
  \]
 The left hand side says the matrix $A$ is an H-representation of the homogeneous cone $C$,
...
\end{verbatim}
The associated H-cone is $C_h((b, -\!A)) = \{ (x_0, x_1, \ldots, x_d)^T\in \R^{d+1}:  b x_0 - A x >=0 \}$,
where $x$ denotes the $d$-dimensional vector $(x_1, \ldots, x_d)^T$.
...
the associated V-cone $C_v((q, R)) = \left\{ 
\begin{pmatrix}x_0 \\ x \end{pmatrix} \in \R^{d+1}:  \begin{pmatrix}x_0 \\ x \end{pmatrix} = 
\begin{pmatrix}q^T \\ R^T\end{pmatrix}  y, \; y >=0 \right\}$,
...
%KF2021 delete the next item as it does not seem to exist.
% \item ZIB Berlin polyhedral software list: \url{ftp://elib.zib-berlin.de/pub/mathprog/polyth/index.html}.
...
%KF2021 update the reference of Minksum
...
%KF2021 update the reference of Gfan
...
%KF2021 update the citatioon fjt-cgf-05 with fjt-cgf-07
...
%KF2021 one added
Mathieu Dutour Sikiri\'c (listed in arbitrary order) and those listed
in the HISTORY file.

\bibliographystyle{plain}

%KF2021 +2 edited below
